...
async def test_auto_ack_strategy_is_selected(pool, redis, strategy):
    strategy("auto_ack")
    # A low-risk skill may be auto-acknowledged after the HITL timeout.
    out = await fb.apply_fallback(
        pool, approval_id="a1", conversation_id="c1", channel="telegram", skill="reply_common"
    )
    assert out["strategy"] == "auto_ack"
    assert out["status"] == ApprovalStatus.APPROVED


async def test_auto_ack_fails_closed_for_unknown_action(pool, redis, strategy):
    strategy("auto_ack")
    # No payload and no skill -> the action is unknown -> timeout must not mint a send.
