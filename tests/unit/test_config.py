...
class TestLoadConfig:
    ...
    def test_domain_defaults(self):
        ...
        assert cfg.domain.numeral_style == "western"

    def test_english_first_defaults(self):
        env = _make_env()
        env.pop("CURRENCY", None)
        env.pop("TENANT_NAME_AR", None)
        env["TENANT_NAME_EN"] = "Test Co"
        cfg = load_config(env)
        assert cfg.domain.currency == "USD"
        assert cfg.domain.reply_language_policy == "english-first"
        assert cfg.tenant.name_en == "Test Co"
        assert cfg.tenant.name_ar == "Test Co"

    def test_numeral_style_override(self):
        env = _make_env()
        env["NUMERAL_STYLE"] = "arabic-indic"
        cfg = load_config(env)
    ...
    def test_hitl_defaults(self):
        ...
        assert cfg.hitl.fallback == "silent"
