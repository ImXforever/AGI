...
def test_payment_with_manager_approval_dispatches_ledger_only() -> None:
    out = execute_action(
        "payment", {"amount": 12400}, actor_role="admin", approved=True, approval_id="rec-pay"
    )
    assert out.executed is True
...
def test_approval_id_cannot_be_replayed() -> None:
    a = execute_action(
        "payment", {"amount": 1}, actor_role="admin", approved=True, approval_id="once"
    )
    b = execute_action(
        "payment", {"amount": 2}, actor_role="admin", approved=True, approval_id="once"
    )
    assert a.executed is True
    assert b.executed is False
    assert "consumed" in b.reason
