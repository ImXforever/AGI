...
class TestTelegram:
    ...
    def test_empty_secret_is_rejected(self):
        result = verify.verify_telegram("", "anything")
        assert result.ok is False and result.reason == "secret not configured"
...
class TestMetaSignature:
    ...
    def test_empty_app_secret_is_rejected(self):
        result = verify.verify_meta_signature("", BODY, None, _meta_sig(BODY))
        assert result.ok is False and result.reason == "secret not configured"
...
class TestMetaHandshake:
    ...
    def test_empty_verify_token_is_rejected(self):
        result = verify.verify_meta_handshake("", "subscribe", "c", "x")
        assert result.ok is False and result.reason == "secret not configured"
...
class TestTwilio:
    ...
    def test_empty_auth_token_is_rejected(self):
        result = verify.verify_twilio_signature("", self.URL, self.PARAMS, "sig")
        assert result.ok is False and result.reason == "secret not configured"
