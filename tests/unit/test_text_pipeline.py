...
class TestInputGuard:
    ...
    def test_absurdly_long_input_is_rejected(self):
        # Guard limit is 4000 characters (long product enquiries are legitimate).
        assert guard.barresi_vorodi("a" * 4001)[0] is False
        assert guard.barresi_vorodi("a" * 3999)[0] is True
...
class TestGuardKeepsLineBreaks:
    def test_multiline_reply_untouched(self):
        text = "Hello!\n\n• one\n• two\n\nAnything else?"
        cleaned, removed = guard.paksazi_khorooji(text)
        assert removed == 0
        assert cleaned == text

    def test_only_tainted_sentence_is_dropped(self):
        text = "Line one is fine.\nSend money via bit.ly/pay now. Second half stays.\nLast line."
        cleaned, removed = guard.paksazi_khorooji(text)
        assert removed == 1
        assert "bit.ly" not in cleaned
        assert "Line one is fine." in cleaned
