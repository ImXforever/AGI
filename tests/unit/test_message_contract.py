...
def test_nested_metadata_is_stripped() -> None:
    result = normalize_envelope(
        _envelope(metadata={"ok": "yes", "blob": {"nested": "no"}, "arr": [1, 2]})
    )
    assert result.metadata == {"ok": "yes"}


def test_attachment_refs_never_include_bytes() -> None:
    from app.core.message_contract import attachment_refs

    refs = attachment_refs(
        [{"filename": "a.pdf", "content_type": "application/pdf", "size": 12, "url": "file-1"}],
        r2_keys=["conv/a.pdf"],
    )
    assert refs[0]["r2_key"] == "conv/a.pdf"
