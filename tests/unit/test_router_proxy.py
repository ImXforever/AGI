...
class _FakeRequest:
    def __init__(self, method="GET", headers=None, body=b"", query="", cookies=None):
        self.method = method
...
def _patch_admin_config(monkeypatch):
    ...
    cfg = _Cfg()
    monkeypatch.setattr(rp, "get_config", lambda: cfg)
    return cfg
...
async def test_endpoint_forwards_a_valid_session_to_the_right_url(monkeypatch):
    cfg = _patch_admin_config(monkeypatch)
    seen: dict = {}
    ...
    # Live sessions are itsdangerous tokens minted by app.admin_api.auth, not the legacy HMAC.
    from app.admin_api.auth import _create_token

    out = await rp.router_proxy(
        _FakeRequest(), "admin/models", petro_session=_create_token(cfg, "admin", is_bootstrap=True)
    )
