...
def test_charter_covers_five_live_domains() -> None:
    ...
    assert snap["version"] == "2.1.0"
    assert snap["product_version"] == "21.0.0"
    assert len(snap["access_matrix"]) == len(ACCESS_MATRIX)
    assert [p.number for p in EXECUTION_PHASES] == [1, 2, 3, 4, 5]
    assert "Never without a manager record" in snap["comparison"][1]["zenovix"]
    assert snap["comparison"][1]["kia"] == snap["comparison"][1]["zenovix"]
...
def test_access_matrix_matches_policy() -> None:
    for action, mode, _label in ACCESS_MATRIX:
        ...
        if mode == "auto":
            ...
            manager = evaluate_action(
                action, actor_role="admin", approved=True, approval_id="rec-1"
            )
...
def test_price_change_on_site_requires_manager() -> None:
    ...
    assert (
        execute_action(
            "change_price", actor_role="admin", approved=True, approval_id="rec-price"
        ).executed
        is True
    )
...
def test_payment_and_delete_never_auto() -> None:
    for action in ("payment", "contract", "delete_data", "change_access"):
        ...
        assert (
            evaluate_action(action, actor_role="agent", approved=True, approval_id="rec-1").allowed
            is False
        )
        assert (
            evaluate_action(action, actor_role="admin", approved=True, approval_id="rec-1").allowed
            is True
        )
...
def test_hermes_drops_unknown_tools() -> None:
    from app.core.hermes_client import _filter_tools_called

    assert _filter_tools_called(["create_ticket", "rm -rf", "payment"]) == [
        "create_ticket",
        "payment",
    ]
