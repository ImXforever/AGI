...
class TestHonestAdmission:
    ...
    def test_admissions_are_recognised_to_avoid_double_wrapping(self):
        ...
        assert orch._is_honest_admission_response(orch._build_honest_admission("fa")) is True
...
class TestOrchestratorGaps:
    def test_unknown_skill_clamps_to_orchestrator(self):
        assert orch._clamp_skill("weather") == orch.SKILL_ORCHESTRATOR
        assert orch._clamp_skill(orch.SKILL_SALES) == orch.SKILL_SALES

    def test_create_ticket_does_not_require_hitl(self):
        assert orch._tools_require_hitl(["create_ticket"]) is False

    def test_quote_and_unknown_tools_require_hitl(self):
        assert orch._tools_require_hitl(["create_quote"]) is True
        assert orch._tools_require_hitl(["explode_server"]) is True

    def test_history_is_truncated_and_skips_junk(self):
        hist = orch._history_for_classify(
            [{"role": "user", "content": "x" * 800}, "nope", {"role": "assistant", "content": "ok"}]
        )
        assert len(hist) == 2
        assert len(hist[0]["content"]) == 500

    def test_source_label_drops_urls(self):
        assert "http" not in orch._sanitize_source_label("see http://127.0.0.1/x msds.pdf")
        assert orch._sanitize_source_label("msds.pdf") == "msds.pdf"
