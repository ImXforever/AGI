...
@pytest.mark.asyncio
async def test_decision_claim_on_an_unknown_item_is_consume_once(rid, _cleanup):
    """Orphan (no meta hash) is claimable exactly once — documented in the Lua script."""
    from app.constants import KEY_HITL_DECIDED, KEY_HITL_META

    aid = f"missing-{rid}"
    _cleanup.extend(
        [KEY_HITL_META.format(approval_id=aid), KEY_HITL_DECIDED.format(approval_id=aid)]
    )
    assert await rs.hitl_claim_decision(aid, status="approved", actor="a") is True
    assert await rs.hitl_claim_decision(aid, status="rejected", actor="b") is False
