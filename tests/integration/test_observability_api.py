...
class TestMetricsEndpoint:
    ...
    async def test_requests_are_counted(self, client: Any):
        ...
        assert "Zenovix-Agent_http_requests_total" in body

    async def test_latency_is_recorded_as_a_histogram(self, client: Any):
        await client.get("/healthz")
        body = (await client.get("/metrics")).text
        assert "Zenovix-Agent_http_request_duration_seconds_bucket" in body
    ...
    async def test_the_json_view_mirrors_the_scrape_endpoint(self, client: Any):
        ...
        assert any(c["name"] == "Zenovix-Agent_http_requests_total" for c in body["counters"])
...
class TestBuildInfo:
    async def test_buildinfo_reports_the_service_version(self, client: Any):
        ...
        assert body["service"] == "Zenovix-Agent-app"
        import app.main as _main
        from app.healthz import router as _hz  # noqa: F401 - ensure module import

        assert body["version"] == _main.app.version
