...
# WHATSAPP_ENABLED=0 in the integration env: the endpoint is deliberately hidden
# (404) so a disabled channel never leaks its existence. With the channel on, a
# bad handshake / signature is 403 (covered by unit tests on verify_meta_*).
_WA_REJECTED = {403, 404}
...
async def test_wa_handshake_rejects_a_wrong_token(client):
    ...
    assert resp.status_code in _WA_REJECTED
