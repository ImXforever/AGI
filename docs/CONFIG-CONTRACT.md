# Zenovix-Agent Platform â€” Configuration Contract
...
| `LEAD_ALERT_ENABLED` | "true" | 1.3.1 — push every website enquiry (lead) to `TELEGRAM_ADMIN_IDS` on Telegram |
| `TELEGRAM_API_BASE` | "https://api.telegram.org" | 1.4.0 — Bot API base URL; change only for a self-hosted Bot API server or a local test harness |
