    ...
    if deep:
        ...
        if cfg is not None:
            ...
            if cfg.llm.mode == "router":
                checks["router"] = {
                    **await _check_http(
                        cfg.llm.router_base_url.replace("/v1", ""), "/v1/models", timeout=2.0
                    ),
                    "critical": False,
                }
            else:
...
_FAVICON_SVG = (
    ...
    b"<rect width='32' height='32' rx='8' fill='#0b0c10'/>"
    b"<path d='M16 5c3 4 6 6.5 6 11a6 6 0 0 1-12 0c0-4.5 3-7 6-11z' fill='#00f0ff'/>"
    b"</svg>"
)
...
async def agent_card() -> dict[str, Any]:
    """A2A discovery card — DropAgent/radius pattern, Zenovix identity."""
    from app.config import config as cfg

    tenant = cfg.tenant.id if cfg else "zenovix"
    return {
        "name": "Zenovix-Agent",
