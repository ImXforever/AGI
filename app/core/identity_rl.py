"""
Zenovix-Agent — Identity reinforcement-learning agent.
...
class IdentityRL:
    ...
    async def _features(self) -> dict[str, float]:
        ...
        row_wd = await pool.fetchrow(
            "SELECT COUNT(*)::int AS cnt FROM orders WHERE customer_id = $1 AND status = 'paid'",
            str(self.user_id),
        )
        n_wd = row_wd["cnt"] if row_wd else 0

        created = (
            float(user["created_at"].timestamp())
