"""
Guardrail — Zenovix-Agent Platform
...
log = get_logger("Zenovix-Agent.guard")
...
def paksazi_dade(matn: str) -> tuple[str, int]:
    ...
    if shomarande:
        log.info("Data sanitized: %d suspicious lines out of %d lines", shomarande, len(khotot))
...
def paksazi_khorooji(javab: str) -> tuple[str, int]:
    ...
    if not MASHKOOK_KHOROOJI.search(javab):
        # Fast path: nothing suspicious — return the text untouched so line
        # breaks, bullets and formatting survive exactly as written.
        return javab, 0

    # Split into sentences but keep the separators, so removing one tainted
    # sentence does not weld the surrounding paragraphs together.
    # A boundary is punctuation followed by whitespace/end, or a line break —
    # so "bit.ly" is not cut in half and the tainted sentence is removed whole.
    parts = re.split(r"((?<=[.!?؟])\s+|\n+)", javab)
    kept: list[str] = []
    hazf = 0
    for i in range(0, len(parts), 2):
        ...
        if sentence and MASHKOOK_KHOROOJI.search(sentence):
            hazf += 1
            continue
