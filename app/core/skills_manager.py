...
Adapted from Hermes DropAgentX skills.py for Zenovix-Agent PostgreSQL backend.
    ...
    *,
    ...
    for name, fm, body, _path in scanned:
        if state.get(name, 1) != 1:
            continue
        scan = scan_skill_text(body, origin="zenovix-agent")
        if scan.blocked:
            log.warning("skill_inject_blocked", extra={"action": "skills.inject", "name": name})
            continue
