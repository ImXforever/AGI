...
async def _words() -> list[str]:
    try:
        pool = await get_pool()
        row = await pool.fetchrow("SELECT value FROM settings WHERE key = 'moderation_enabled'")
    ...
    try:
        pool = await get_pool()
        row = await pool.fetchrow("SELECT value FROM settings WHERE key = 'moderation_extra_words'")
