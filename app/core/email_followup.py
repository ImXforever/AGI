...
def followup_from_row(row: dict[str, Any]) -> FollowUpEntry:
    """Map an email_followups table row to FollowUpEntry."""
    status_raw = str(row.get("status") or FollowUpStatus.ACTIVE)
    try:
    ...
    return FollowUpEntry(
        ...
        rule_name=str(row.get("rule_name") or "standard"),
        status=status,
        created_at=float(row.get("created_at") or 0),
        last_followup_at=(
            float(row["last_followup_at"]) if row.get("last_followup_at") is not None else None
        ),
        followup_count=int(row.get("followup_count") or 0),
        next_followup_at=float(row.get("next_followup_at") or 0),
        escalation_at=float(row.get("escalation_at") or 0),
    )
