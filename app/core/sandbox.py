...
Adapted from Hermes DropAgentX sandbox.py for Zenovix-Agent.
...
def classify_command(command: str) -> CommandClassification:
    ...
    for pattern in SAFE_PATTERNS:
        if cmd == pattern or (
            cmd.startswith(pattern + " ") and re.match(rf"^{re.escape(pattern)}\s", cmd)
        ):
...
async def run_shell(
    ...
    *,
    ...
    if result.tier == "guard":
        ...
        # Production never skips the HITL gate (yolo / allow-all are dev-only).
        if not _bypass_guard_allowed():
            yolo = False
            if approval_mode == "allow-all":
                approval_mode = "auto"
...
async def _local_shell(command: str) -> str:
    ...
    if any(op in command for op in shell_metacharacters):
...
def _bypass_guard_allowed() -> bool:
    """yolo_mode / allow-all are never honoured in production."""
