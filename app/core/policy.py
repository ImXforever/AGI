...
def _context_sensitivity(context: dict[str, Any] | None) -> str:
    if not context:
        return ""
    raw = (
        str(context.get("sensitivity") or context.get("category") or context.get("kind") or "")
        .strip()
        .lower()
    )
    return raw
...
def evaluate_action(
    ...
    *,
    ...
    if (
        normalized == "publish_content"
        and ctx.get("calendar_scheduled")
        and sensitivity not in _SENSITIVE_CONTEXT
    ):
    ...
    if sensitivity in _SENSITIVE_CONTEXT and normalized in {
        "reply_common",
        "publish_calendar",
        "create_draft",
    }:
        ...
        normalized = (
            "send_email"
            if "email" in action.lower() or normalized == "reply_common"
            else normalized
        )
    ...
    if not str(approval_id).strip():
        return PolicyDecision(normalized, risk, False, True, "approval record id is required")
