...
from app.core.support_workflow import is_safety_text
...
# BUG #6 (1.3.1): the LLM planner, the MCP bridge and /tools/execute all pass
# free-text severities ("urgent", "medium", "P1", "emergency"...). Anything
# outside the four canonical values hit ``tickets_severity_check`` and the
# whole tool call failed — the customer's problem was silently dropped.
# Normalise every synonym onto the CHECK constraint before the INSERT.
_SEVERITY_ALIASES = {
    "low": "low",
    "minor": "low",
    "trivial": "low",
...
def normalize_severity(value: object, *, default: str = "normal") -> str:
    """Map any severity spelling onto ``low|normal|high|critical``.

    Unknown or empty values fall back to *default* instead of raising, so a
    creative model output can never break ticket creation.
    """
...
async def create_ticket(
    ...
    *,
    ...
    """
    is_safety = is_safety_text(f"{subject} {body}")
    severity = normalize_severity(severity)
    if is_safety and severity in ("low", "normal"):
        severity = "high"
    ...
    "create_ticket": {
        ...
        "params": {
            ...
            "severity": {
                ...
                "description": "low | normal | high | critical (synonyms such as urgent/medium are mapped)",
            },
