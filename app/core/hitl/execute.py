...
def execute_action(
    ...
    *,
    ...
    record = str(approval_id or "").strip()
    if record and record in _USED_APPROVALS:
    ...
    try:
        ...
        log.exception(
            "handler_failed", extra={"action": "hitl.execute", "business_action": policy.action}
        )
        outcome = ExecutionResult(
            ok=False,
            action=policy.action,
