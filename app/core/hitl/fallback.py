...
COURTESY_TIMEOUT_EN = "A manager will reply during business hours. Your request was recorded."
...
def may_auto_ack(payload: Any = None, skill: str = "") -> bool:
    ...
    Unknown / empty action fails closed — timeout must not mint a send.
    """
    action = ""
    ...
    if isinstance(payload, dict):
        action = str(payload.get("action") or payload.get("type") or payload.get("intent") or "")
    token = (action or skill or "").strip().lower().split(".")[-1]
    if not token:
        return False
    if token in _NEVER_AUTO_ACK or token.endswith("_agent") or token == "orchestrator":  # nosec B105 - role token, not a credential
        return False
    from app.core.policy import approval_actions

    if token in approval_actions():
        return False
    return True
    ...
    *,
    ...
    if strategy == "auto_ack" and not may_auto_ack(payload, skill):
        log.warning(
            ...
            extra={
                "action": "hitl.fallback.force_silent",
...
async def _silent_reject(
    ...
    *,
    ...
    """Reject the draft. Send a fixed courtesy line — never the draft."""
    actor = "system:silent"
    ...
    courtesy = False
    if channel and customer_id:
        courtesy = await _deliver_fallback_message(
            channel=channel,
            recipient_id=customer_id,
            text=COURTESY_TIMEOUT_EN,
            registry=registry,
        )

    log.info(
        "fallback silent applied",
        extra={"action": "hitl.fallback.silent", "approval_id": approval_id, "courtesy": courtesy},
    )
    return {
        "approval_id": approval_id,
        "strategy": "silent",
