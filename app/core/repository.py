...
async def get_or_create_customer(
    ...
    *,
    ...
    try:
        ...
        # Unique (channel, external_id) under 10k concurrent ingest.
        if "unique" not in str(exc).lower() and "UniqueViolation" not in type(exc).__name__:
            raise
        row = await pg.fetchrow(
            "SELECT * FROM customers WHERE channel = $1 AND external_id = $2",
            channel,
...
async def store_message(
    ...
    *,
    ...
    try:
        ...
        # PRODUCTION BUG (1.2.0): idx_messages_conv_extref is UNIQUE on
        # (conversation_id, external_ref). Telegram numbers messages per *bot*,
        # so a new bot token (or a Redis flush followed by a Telegram retry)
        # re-sends message_ids that already exist for this chat. The
        # UniqueViolation escaped ingest_incoming → webhook 503 → Telegram
        # retried → the retry was dropped as a duplicate → the customer never
        # got a reply. A colliding ref is a real, *new* message from the
        # customer, so keep it: re-insert under a unique ref derived from the
        # original one instead of failing the whole ingest.
        if "unique" not in str(exc).lower() and "UniqueViolation" not in type(exc).__name__:
            raise
        if not external_ref:
            raise
        salvaged_ref = f"{external_ref}#{mid[:8]}"
        log.warning(
            "message_external_ref_collision",
            extra={
                "action": "store_message",
                "conversation_id": conversation_id,
                "external_ref": external_ref,
                "salvaged_ref": salvaged_ref,
            },
        )
        await pg.execute(
            """
