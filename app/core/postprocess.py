...
from app.core.catalog_context import strip_tool_protocol
...
def run(text: str, *, config: Config | None = None) -> PostprocessReport:
    ...
    Steps:
        ...
        2. Convert Markdown to Telegram HTML (bold only, bullets, no tables)
        3. Sanitize (control chars, bidi, whitespace) + strip leftovers
    ...
    protocol_free = strip_tool_protocol(text)
    if protocol_free != text:
        fixes.append("strip_tool_protocol")
        text = protocol_free
    ...
    # Model Markdown (**bold**, ## headings, ---, tables, fences) is converted
    # into Telegram HTML here, once, for every channel. Telegram renders it;
    # the WhatsApp and e-mail adapters translate Telegram HTML to their own
    # plain formats. Customers never see raw stars/hashes again.
    from app.core.reply_format import markdown_to_telegram_html

    formatted = markdown_to_telegram_html(text)
    if formatted != text:
        fixes.append("markdown_to_html")
        text = formatted
