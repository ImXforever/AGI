...
def create_calendar_post(
    ...
    *,
    ...
    from app.core.social_workflow import caption_is_ad_spend, caption_is_ordinary

    if caption_is_ad_spend(caption):
        raise ValueError("ad spend is not allowed")
    ...
    return CalendarPost(
        ...
        status=(
            CalendarPostStatus.SCHEDULED
            if caption_is_ordinary(caption)
            else CalendarPostStatus.DRAFT
        ),
...
def is_ready_to_publish(post: CalendarPost) -> bool:
    """Check if a post should be published now. Sensitive/ads never auto-fire."""
    if post.status != CalendarPostStatus.SCHEDULED:
        return False
    if time.time() < post.scheduled_at:
        return False
    from app.core.social_workflow import caption_is_ordinary

    return caption_is_ordinary(post.caption)
...
def calendar_post_from_row(row: dict[str, Any]) -> CalendarPost:
    """Map a content_calendar table row to CalendarPost."""
    tags = row.get("hashtags") or []
    if isinstance(tags, str):
        tags = []
    status_raw = str(row.get("status") or CalendarPostStatus.DRAFT)
    try:
        status = CalendarPostStatus(status_raw)
    except ValueError:
        status = CalendarPostStatus.DRAFT
    platform_raw = str(row.get("platform") or ContentPlatform.INSTAGRAM)
    try:
        platform = ContentPlatform(platform_raw)
    except ValueError:
    ...
    return CalendarPost(
        ...
        platform=platform,
        scheduled_at=float(row.get("scheduled_at") or 0),
        status=status,
        created_at=float(row.get("created_at") or 0),
        published_at=(float(row["published_at"]) if row.get("published_at") is not None else None),
        post_id=str(row["post_id"]) if row.get("post_id") else None,
        media_url=str(row.get("media_url") or ""),
        hashtags=tuple(str(t) for t in tags),
        created_by=str(row.get("created_by") or "admin"),
