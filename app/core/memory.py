"""Long-term user memory — Zenovix-Agent memory providers + purchase profile.
...
async def recommend_for_user(user_id: int, limit: int = 5) -> list[dict]:
    ...
    # exist in Zenovix-Agent (products.title / price_credits / creator_id /
...
async def my_memory_summary(user_id: int) -> tuple[str, int]:
    ...
    if prof["buys"]:
        ...
        lines.append(
            ...
            + (f" · Categories: {', '.join(prof['categories'][:4])}" if prof["categories"] else "")
        )
    return "\n".join(lines), total
