...
class LLMClient:
    ...
    def _mock_orchestrator(self, user_text: str, *, purpose: str = "general") -> str:
        ...
        if purpose == "classification":
            if any(
                w in lower
                for w in ("مرحبا", "السلام", "أهلا", "سلام", "درود", "hi", "hello", "hey")
            ):
                ...
                w in lower
                for w in ("دعم", "مشكلة", "error", "bug", "issue", "support", "پشتیبانی", "خراب")
            ):
                ...
                w in lower
                for w in ("تقرير", "إحصا", "report", "analytics", "stat", "آمار", "گزارش")
            ):
    ...
    def _mock_skill_reply(self, text: str, lang: str = "en") -> str:
        if lang == "fa":
            ...
            if any(w in text for w in ("محصول", "کالا", "product")):
                return (
                    "ما سبد متنوعی از محصولات صنعتی ارائه می‌دهیم. می‌توانید نام محصول را جستجو کنید."
                )
        ...
        if any(w in text for w in ("support", "issue", "bug", "help")):
            return (
                "I understand your request. I am logging a support ticket with our technical team."
            )
