...
Public: user web app, manager login form, TWA probe/login, password auth.
"""
...
_PUBLIC_EXACT = {
    ...
    # 1.5.0: the login page draws the 3D background with the vendored
    # three.js (a public library, no business data) — it must load before
    # sign-in, so it is the one static asset outside the gate.
    "/admin/ops/vendor/three.min.js",
    "/admin/twa.html",
}
...
class AdminGateMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if wants_html and request.method in ("GET", "HEAD"):
            return RedirectResponse(UNAUTH_HTML_REDIRECT, status_code=302)
