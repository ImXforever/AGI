"""Email channel adapter — Resend or Twilio SendGrid (outbound), SendGrid / IMAP (inbound)."""
...
DEFAULT_SENDGRID_URL = "https://api.sendgrid.com/v3"
...
class EmailAdapter:
    ...
    def __init__(self, cfg: Config, redis: Any) -> None:
        ...
        self._provider = getattr(cfg.channels, "email_provider", "resend")
        self._sendgrid_key = cfg.channels.sendgrid_api_key
        self._sendgrid_url = DEFAULT_SENDGRID_URL
    ...
    async def send(
        ...
        **kwargs: Any,
        ...
        """Send an email via Resend or Twilio SendGrid (``EMAIL_PROVIDER``).
        ...
        # Replies are authored for chat channels (Telegram HTML); mail wants plain text.
        text = telegram_html_to_plain(text)
        subject = kwargs.get("subject", "") or self._default_subject(reply_to_ref)
    ...
    def _default_subject(self, reply_to_ref: str | None) -> str:
        """Subject line when the orchestrator passes none (it never does today)."""
        name = self._cfg.tenant.name_en or self._cfg.tenant.name_ar
        return f"Re: your enquiry — {name}" if reply_to_ref else f"{name} — reply to your enquiry"
    ...
    async def _send_sendgrid(
        ...
        *,
        ...
        """Twilio SendGrid v3 Mail Send. Returns the X-Message-Id header as external_ref."""
        personalization: dict[str, Any] = {"to": [{"email": recipient_id}]}
        payload: dict[str, Any] = {
        ...
        if reply_to_ref:
            payload["headers"] = {"In-Reply-To": reply_to_ref, "References": reply_to_ref}
        if attachments:
            sg_attachments = [
                {
                    "filename": att.filename,
        ...
        try:
            resp = await self._http.post(
                f"{self._sendgrid_url}/mail/send", json=payload, headers=headers
            )
            resp.raise_for_status()  # SendGrid answers 202 with an empty body
            ...
            return OutboundResult(
                success=True,
                external_ref=message_id,
                metadata={"to": recipient_id, "subject": subject, "provider": "sendgrid"},
            )
        except httpx.HTTPStatusError as exc:
            error_body = exc.response.text
            log.error(
    ...
    def _poll_imap_sync(self) -> list[IncomingMessage]:
        ...
        if not (cfg.imap_host and cfg.imap_user and cfg.imap_password) or str(
            cfg.imap_user
        ).startswith("PASTE_"):
            # Mailbox not configured yet — nothing to poll, nothing to alarm about.
            log.debug("imap not configured — poll skipped", extra={"action": "imap.poll"})
            return messages
        ...
        try:
            ...
            log.warning("imap connect failed", extra={"action": "imap.connect", "error": str(exc)})
            return messages
        ...
        try:
            ...
            log.warning("imap poll failed", extra={"action": "imap.poll", "error": str(exc)})
        finally:
    ...
    async def run_imap_poller(self, services: dict[str, Any]) -> None:
        """Background loop: poll the mailbox and hand new mail to the pipeline.

        Started from the app lifespan when ``EMAIL_INBOUND_PROVIDER=imap``. Each
        message goes through the same ``ingest_incoming`` path as a webhook, so
        dedup / rate-limit / security / persistence behave identically.
        """
        import asyncio

        from app.core.pipeline import ingest_incoming

        interval = max(5, int(self._cfg.channels.imap_poll_seconds))
        log.info(
            "imap poller started",
            extra={"action": "imap.start", "host": self._cfg.channels.imap_host, "every": interval},
        )
        while True:
            try:
                for incoming in await self.poll_imap_once():
                    try:
                        await ingest_incoming(
                            ...
                            reply_to_ref=incoming.reply_to_ref,
                        ...
                        log.error(
                            "imap ingest failed",
                            extra={"action": "imap.ingest", "error": str(exc)},
                        )
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                log.error("imap poll loop error", extra={"action": "imap.poll", "error": str(exc)})
