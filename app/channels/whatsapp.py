...
class WhatsAppAdapter:
    ...
    async def send(
        ...
        **kwargs: Any,
        ...
        # Menus/templates are authored in Telegram HTML; WhatsApp renders
        # *bold* / _italic_ / `code` instead, and shows raw tags otherwise.
        clean = normalize_text(telegram_html_to_plain(text, bold="*", italic="_", code="`"))
    ...
    # ── media download (1.6.0) ──────────────────────────────────────

    async def download_attachment(
        self, ref: str, *, max_bytes: int = 10 * 1024 * 1024
    ) -> bytes | None:
        """Fetch the bytes of an inbound media item so its text can be read.

        1.5.0 taught the pipeline to read customer documents, but only the
        Telegram adapter could download them — a PDF sent on WhatsApp was
        archived by reference and silently ignored. ``ref`` is what the
        parsers put in ``Attachment.url``: a Meta media *id* (resolved through
        ``GET /{media-id}`` → ``url`` → authenticated download) or a Twilio
        ``MediaUrl`` (basic-auth download). Never raises.
        """
        ...
        try:
            if self._provider == "meta":
                ...
                if ref.startswith("http"):
                    ...
                    meta = await self._http.get(
                        f"{self._cfg.channels.whatsapp_base_url}/{ref}", headers=headers
                    )
                    meta.raise_for_status()
                    media_url = str(meta.json().get("url") or "")
                    if not media_url:
                        return None
                resp = await self._http.get(media_url, headers=headers, follow_redirects=True)
            else:
                ...
                resp = await self._http.get(
                    ref,
                    auth=(
                        self._cfg.channels.twilio_account_sid,
                        self._cfg.channels.twilio_auth_token,
                    ),
            ...
            if len(data) > max_bytes:
                log.warning(
                    "wa media too large",
                    extra={"action": "download", "channel": CHANNEL_WHATSAPP, "size": len(data)},
                )
                return None
            return data
        except Exception as exc:
            log.warning(
                "wa media download failed",
                extra={"action": "download", "channel": CHANNEL_WHATSAPP, "error": str(exc)[:200]},
            )
            return None
