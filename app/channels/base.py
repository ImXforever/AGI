...
_TG_TAG_RE = re.compile(
    r"</?(b|strong|i|em|u|ins|s|strike|del|code|pre|a|br|tg-spoiler|span|blockquote)\b[^>]*>",
    re.IGNORECASE,
)


def telegram_html_to_plain(text: str, *, bold: str = "", italic: str = "", code: str = "") -> str:
    """Convert Telegram-flavoured HTML (menus, templates) for plain channels.

    Only the tags Telegram itself allows are touched, so a literal ``<10 ppm``
    in a product answer survives. ``bold``/``italic``/``code`` are the markers
    to emit instead (WhatsApp uses ``*``, ``_`` and backticks; email uses none).
    """
    ...
    return html.unescape(_TG_TAG_RE.sub(_swap, text))
