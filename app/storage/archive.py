...
def pg_sslmode_for(dsn: str, explicit: str | None = None) -> str:
    """Pick the libpq ``sslmode`` for pg_dump.

    BUG #9 (1.3.1): 1.3.0 hard-coded ``PGSSLMODE=require``. Railway's internal
    ``postgres.railway.internal`` endpoint, docker-compose and every local
    Postgres speak plain TCP, so the nightly backup died with
    ``server does not support SSL`` and no archive was ever written.
    Priority: explicit ``PGSSLMODE`` env → ``?sslmode=`` in DATABASE_URL →
    ``prefer`` (TLS when the server offers it, plain otherwise).
    """
    ...
    from_url = (query.get("sslmode") or [""])[0].strip()
    if from_url:
        return from_url
    return "prefer"
...
async def _pg_dump_bytes() -> bytes:
    ...
    env = os.environ.copy()
    env["PGSSLMODE"] = pg_sslmode_for(dsn, os.environ.get("PGSSLMODE"))
    if parsed.password:
        env["PGPASSWORD"] = unquote(parsed.password)
    # Postgres URLs may carry libpq options (sslmode, sslrootcert, ...).
    for key, values in parse_qs(parsed.query).items():
        if key == "sslmode" or not values:
            continue
        env.setdefault(f"PG{key.upper()}", values[0])
