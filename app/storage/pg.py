...
async def connect_pg() -> asyncpg.Pool:
    ...
    _pool = await asyncpg.create_pool(
        ...
        max_size=10,
        command_timeout=30,
        server_settings={"application_name": "Zenovix-Agent"},
    )
            ...
            for version, name, sql_text in migrations:
                if version in applied_map:
                    ...
                    if existing_checksum != current_checksum:
                        # An already-applied migration file changed on disk (usually a
                        # comment / whitespace edit in a later release). Refusing to boot
                        # would take the live bot down for a cosmetic diff, so by default
                        # we log once and continue. Set MIGRATION_CHECKSUM_STRICT=true to
                        # make this fatal (recommended only for fresh environments).
                        if _b_env("MIGRATION_CHECKSUM_STRICT", False):
                            raise RuntimeError(
                                f"Migration {version} ({name}) checksum mismatch — "
                                "refusing to continue (MIGRATION_CHECKSUM_STRICT=true). "
                        ...
                        log.warning(
                            ...
                            extra={
                                ...
                                "hint": (
                                    ...
                                    "continuing (set MIGRATION_CHECKSUM_STRICT=true to fail)"
                                ),
                            },
                        )
                    log.debug(
