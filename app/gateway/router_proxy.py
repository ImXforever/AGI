...
def _validate_admin_session(session_token: str | None, web_secret: str) -> bool:
    """Legacy HMAC cookie (ts:sig). Not used for live sessions.

    Live ``pa_session`` is itsdangerous via ``app.admin_api.auth._read_token``.
    Do not mix the two; HMAC never matches a real admin cookie.
    """
...
async def router_proxy(
    request: Request,
    path: str,
    petro_session: str | None = Cookie(None, alias="pa_session"),
) -> Response:
    ...
    token = petro_session or request.cookies.get("pa_session")
    if _read_token(cfg, token or "") is None:
