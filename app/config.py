"""Zenovix-Agent Platform — the ONLY module that reads os.environ."""
...
class ChannelsGroup:
    ...
    # 1.4.0: Bot API base (default api.telegram.org). Override only for a
    # local proxy / self-hosted Bot API server or an offline test harness.
    ...
    twilio_base_url: str
    email_enabled: bool
    email_provider: Literal["resend", "sendgrid"]
    email_from: str
    email_reply_to: str
    resend_api_key: str
    resend_base_url: str
...
class DomainGroup:
    ...
    # 1.3.1 option: push a Telegram alert to TELEGRAM_ADMIN_IDS the moment a
    # website enquiry (lead) arrives. Default on; LEAD_ALERT_ENABLED=false
    # switches it off without touching the enquiry flow itself.
    lead_alert_enabled: bool = True
...
def load_config(env: dict[str, str] | None = None) -> Config:
    ...
    tenant = TenantGroup(
        ...
        or _s(source, "TENANT_NAME_AR", default="")
        or _s(source, "TENANT_ID", required=True),
        name_ar=_s(source, "TENANT_NAME_AR", default="")
        or _s(source, "TENANT_NAME_EN", default="")
        or _s(source, "TENANT_ID", required=True),
        support_contact=_s(source, "SUPPORT_CONTACT", required=True),
        brand_logo_url=_s(source, "BRAND_LOGO_URL"),
        brand_primary_color=_s(source, "BRAND_PRIMARY_COLOR", default="#00f0ff"),
        timezone=_s(source, "TENANT_TIMEZONE", default="Europe/Amsterdam"),
        business_hours=_s(source, "BUSINESS_HOURS", default="09:00-17:00"),
    )
    ...
    # Outbound provider. ``sendgrid`` = Twilio SendGrid (same Twilio console as
    # WhatsApp); ``resend`` kept as the historical default.
    email_provider = _one_of(source, "EMAIL_PROVIDER", ("resend", "sendgrid"), "resend")
    ...
    channels = ChannelsGroup(
        ...
        telegram_api_base=_s(
            source, "TELEGRAM_API_BASE", default="https://api.telegram.org"
        ).rstrip("/"),
        ...
        email_provider=email_provider,  # type: ignore[arg-type]
        ...
        resend_api_key=_require_if(email_enabled and email_provider == "resend", "RESEND_API_KEY"),
        ...
        sendgrid_api_key=_require_if(
            email_enabled and (email_provider == "sendgrid" or email_inbound == "sendgrid"),
            "SENDGRID_API_KEY",
        ),
    ...
    if (
        email_enabled
        and email_inbound != "imap"
        and not channels.sendgrid_webhook_public_key
        and not channels.resend_webhook_secret
    ):
        raise ConfigError(
            "At least one of SENDGRID_WEBHOOK_PUBLIC_KEY or RESEND_WEBHOOK_SECRET "
            "is required when EMAIL_ENABLED=true and inbound is a webhook "
            "(set EMAIL_INBOUND_PROVIDER=imap to poll a mailbox instead)"
        )
    ...
    hitl = HITLGroup(
        ...
        fallback=_one_of(source, "HITL_FALLBACK", ("auto_ack", "silent"), "silent"),  # type: ignore[arg-type]
    ...
    domain = DomainGroup(
        currency=(_s(source, "CURRENCY", default="USD") or "USD").upper(),
        ...
        numeral_style=_one_of(source, "NUMERAL_STYLE", ("arabic-indic", "western"), "western"),  # type: ignore[arg-type]
        reply_language_policy=_s(source, "REPLY_LANGUAGE_POLICY", default="english-first"),
        lead_alert_enabled=_b(source, "LEAD_ALERT_ENABLED", True),
    )
...
def _is_secret_field(name: str) -> bool:
    lowered = name.lower()
    return any(part in lowered for part in _SECRET_NAME)


def as_admin_settings(cfg: Config) -> dict[str, Any]:
    """Full configuration for the manager console. Secrets are masked, never omitted."""
    from dataclasses import asdict, is_dataclass

    def walk(obj: Any, key: str = "") -> Any:
        if is_dataclass(obj) and not isinstance(obj, type):
            return {k: walk(getattr(obj, k), k) for k in obj.__dataclass_fields__}
        if isinstance(obj, dict):
            return {k: walk(v, k) for k, v in obj.items()}
        if isinstance(obj, (list, tuple)):
            if key and _is_secret_field(key):
                return [_mask_secret(str(v)) for v in obj]
            return list(obj)
        if isinstance(obj, str) and _is_secret_field(key):
...
def as_redacted_dict(cfg: Config) -> dict[str, Any]:
    settings = as_admin_settings(cfg)
    groups = settings["groups"]
    return {
        "tenant": {
            "id": cfg.tenant.id,
            "name_ar": cfg.tenant.name_ar,
            "name_en": cfg.tenant.name_en,
        ...
        "admin": {
            ...
            "token_masked": _mask_secret(cfg.admin.web_secret),
        },
