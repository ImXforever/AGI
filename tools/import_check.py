...
def discover_modules() -> list[str]:
    ...
    for py_file in sorted(app_dir.rglob("*.py")):
        if py_file.name == "__init__.py":
            ...
            # BUG #7 (1.3.1): ``str(rel).with_suffix`` is an AttributeError —
            # the CI gate crashed on its very first file since 1.2.0, so the
            # import smoke-test never actually ran.
...
def main() -> None:
    print("=" * 60)
    print("Zenovix-Agent Import Health Check")
    print("=" * 60)
