...
ALLOWLIST_PATTERNS = [
    r"\.env\.example",
    r"PASTE_",  # railway-variables template placeholders
    r"=\s*cfg\.",  # config attribute wiring, e.g. api_secret=cfg.channels.twitter_api_secret
    r"form\.password\.value",  # browser form field reference in the admin login page
    r"\.password\b",  # attribute access, not a literal
...
def scan() -> list[Finding]:
    ...
    for path in ROOT.rglob("*"):
        if path.is_dir():
            continue
        # rglob descends into every directory; skip files whose path crosses a SKIP_DIRS entry
...
def main() -> None:
    print("=" * 60)
    print("Zenovix-Agent Secret Scanner")
    print("=" * 60)
