-- =============================================================================
-- PetroAgent Platform — Complete Database Schema (16 tables)
-- PostgreSQL 16+
-- =============================================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_customers_channel_external ON customers (channel, external_id);
...
-- ticket_notes (sub-table for tickets)
...
-- ---------------------------------------------------------------------------
-- 10. orders_imports (staging for external order CSV/JSON imports)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Trigger: auto-increment hit_count on faq and troubleshooting
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION increment_hit_count()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_TABLE_NAME = 'faq' THEN
        UPDATE faq SET hit_count = hit_count + 1 WHERE id = NEW.id;
    ELSIF TG_TABLE_NAME = 'troubleshooting' THEN
        UPDATE troubleshooting SET hit_count = hit_count + 1 WHERE id = NEW.id;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
